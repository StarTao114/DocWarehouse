...
unsigned char Freq[3] = { 0x6C, 0x80, 0x00 };	//434000000 
...
void  SX1276LoRaSetRFFrequency(void) {
	SX1276WriteBuffer( REG_LR_FRFMSB, Freq[0]);//0x04射频载波频率最高有效位
	SX1276WriteBuffer( REG_LR_FRFMID, Freq[1]);//0x07射频载波频率中间有效位
	SX1276WriteBuffer( REG_LR_FRFLSB, Freq[2]);//0x00射频载波频率最低有效位
}
